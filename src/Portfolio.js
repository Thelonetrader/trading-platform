  ...
  const tabBtn = (id, label) => (
    <button
      ...
      style={{
        ...
        fontWeight: 600,
        fontSize: 13,
        background: tab === id ? '#6366f1' : 'var(--tp-bg-active)',
  ...
  return (
    <div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          >
            {[
              ...
              >
                <div style={{ fontSize: 11, color: 'var(--tp-text-faint)', textTransform: 'uppercase', marginBottom: 4 }}>{s.label}</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: s.color || '#f1f5f9' }}>{s.value}</div>
              </div>
            ))}
          </div>
          {connection?.status !== 'connected' && (
            <div style={{ color: 'var(--tp-text-muted)', fontSize: 12, marginBottom: 12 }}>
              Connect IB to include live broker positions and quotes on this view.
            </div>
          )}
          {loadingBroker && connection?.status === 'connected' && (
            <div style={{ color: 'var(--tp-text-muted)', fontSize: 13, marginBottom: 12 }}>Refreshing IB positions…</div>
          )}
          {unifiedRows.length === 0 ? (
            <div style={{ color: 'var(--tp-text-muted)', fontSize: 13 }}>Add manual holdings or connect IB with open positions.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {unifiedRows.map((row) => (
                ...
                >
                  ...
                  <div>
                    <span style={{ fontWeight: 800, color: 'var(--tp-text-title)' }}>{row.ticker}</span>
                    {row.name && <span style={{ marginLeft: 8, fontSize: 12, color: 'var(--tp-text-muted)' }}>{row.name}</span>}
                  </div>
                  <div style={{ fontSize: 13, color: 'var(--tp-text-secondary)' }}>{row.shares} sh</div>
                  <div style={{ fontSize: 13, color: 'var(--tp-text-muted)' }}>
                    {row.avgCost.toFixed(2)} {row.currency}
                  </div>
                  <div style={{ fontSize: 13, color: 'var(--tp-text)' }}>
                    {row.price.toFixed(2)}
      ...
      {tab === 'broker' && (
        <div style={{ marginBottom: 24 }}>
          {connection?.status !== 'connected' ? (
            ...
            <div style={{ color: 'var(--tp-text-muted)', fontSize: 13 }}>Loading positions…</div>
          ) : brokerPositions.length === 0 ? (
            <div style={{ color: 'var(--tp-text-muted)', fontSize: 13 }}>No open IB positions.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {brokerPositions.map((p, i) => (
                ...
                >
                  <div>
                    <span style={{ fontWeight: 800, color: 'var(--tp-text-title)' }}>{p.symbol}</span>
                    <span style={{ marginLeft: 10, fontSize: 12, color: 'var(--tp-text-muted)' }}>{p.exchange}</span>
                  </div>
                  <div style={{ textAlign: 'right', fontSize: 13 }}>
                    <div style={{ color: 'var(--tp-text)' }}>{p.position} sh</div>
                    <div style={{ color: 'var(--tp-text-muted)', fontSize: 12 }}>Avg {Number(p.avgCost).toFixed(2)} {p.currency}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {tab === 'manual' && (
        <>
      {connection?.status === 'connected' && (
        <div style={{ fontSize: 12, color: 'var(--tp-text-muted)', marginBottom: 16 }}>
          When IB is connected, <strong style={{ color: 'var(--tp-text-secondary)', fontWeight: 600 }}>Current</strong> uses live
      ...
      {showForm && (
        <div style={{
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--tp-text-title)', marginBottom: 20 }}>Add Holding</div>
      ...
      {holdings.length === 0 ? (
        <div style={{
          ...
          <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--tp-text-faint)', marginBottom: 6 }}>No holdings yet</div>
              ...
              {holdings.map((h, i) => (
                <tr key={h.id} style={{
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--tp-text-title)' }}>{h.ticker}</div>
                    {h.name && <div style={{ fontSize: 11, color: 'var(--tp-text-faint)' }}>{h.name}</div>}
                  </td>
                  <td style={{ padding: '12px 16px', fontSize: 12, color: 'var(--tp-text-muted)' }}>{formatSectorDisplay(h.sector) || '—'}</td>
                  <td style={{ padding: '12px 16px', fontSize: 13, color: 'var(--tp-text-secondary)' }}>{h.shares}</td>
                  <td style={{ padding: '12px 16px', fontSize: 13, color: 'var(--tp-text-secondary)' }}>£{h.avgBuyPrice}</td>
                  <td style={{ padding: '12px 16px', fontSize: 13, color: 'var(--tp-text-secondary)' }}>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: 13, fontWeight: 600, color: 'var(--tp-text-title)' }}>£{calcValue(h)}</td>
